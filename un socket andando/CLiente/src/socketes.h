...
//crearAddr recibe el puerto y la ip(formatos:"1.123.122" y in_addr_t(para poder usar el INADDR_ANY))
struct sockaddr_in crearAddr(uint16_t puerto,char *ip){

	struct sockaddr_in un_addr;
		un_addr.sin_family=AF_INET;
		un_addr.sin_port=htons(puerto); //fijarse q funciona bien el tipo
		un_addr.sin_addr.s_addr=htonl(INADDR_ANY);//esto se cambia
...
//bindea un socket a un puerto y una ip (soporta ambos formatos)
